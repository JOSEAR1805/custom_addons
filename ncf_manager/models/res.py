...
class ResPartner(models.Model):
    ...
    def write(self, vals):
        if vals.get("name", False):
            partner_id = self.search([('name', '=', vals.get("name", False)), ('vat', '=', vals.get("vat", False))])
    ...
    def create(self, vals):
        ...
        if vals.get("name", False):
            partner_id = self.search([('name', '=', vals["name"]), ('vat', '=', vals.get("vat"))])
            if not self._context.get("job_uuid", False):
                if partner_id:
                    raise exceptions.ValidationError(
                        u"Ya existe un contacto registrado con este nombre de {}! incluir otro apellido o informacion"
    ...
    def rewrite_due_date(self):
        for rec in self:
            ...
            for inv in invoice_ids:
                ...
                if pterm:
                    pterm_list = pterm.with_context(currency_id=inv.company_id.currency_id.id).compute(value=1,
                                                                                                       date_ref=inv.date_invoice)[
                        0]
